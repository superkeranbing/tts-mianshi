...
class InterviewReport(Base):
    ...
    recording_id: Mapped[str] = mapped_column(Text, ForeignKey("recordings.id", ondelete="CASCADE"), nullable=False)
    resume_id: Mapped[str | None] = mapped_column(Text, ForeignKey("resumes.id", ondelete="SET NULL"), nullable=True)
    ...
    improvement_plan_json: Mapped[str] = mapped_column(Text, default="[]")
...
class QAPair(Base):
    ...
    id: Mapped[str] = mapped_column(Text, primary_key=True, default=lambda: str(uuid.uuid4()))
    report_id: Mapped[str] = mapped_column(Text, ForeignKey("interview_reports.id", ondelete="CASCADE"), nullable=False)
...
class KnowledgePoint(Base):
    ...
    interview_tips_json: Mapped[str] = mapped_column(Text, default="[]")
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
